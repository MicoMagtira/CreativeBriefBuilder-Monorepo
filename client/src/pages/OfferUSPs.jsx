...
const OfferUSPs = () => {
  ...
  const [errors, setErrors] = useState({ offer: '', usps: '', formats: '' });

  const validate = () => {
    let valid = true;
    const newErrors = { offer: '', usps: '', formats: '' };
    if (!formData.offer.trim()) {
      newErrors.offer = 'Offer is required.';
      valid = false;
    }
    // Only count non-empty, trimmed USPs
    const uspCount = formData.usps.filter((u) => u.trim()).length;
    if (uspCount < 3) {
      newErrors.usps = 'Please provide at least 3 unique selling points.';
      valid = false;
    }
    if (!Object.values(formData.selectedFormats).some((v) => v)) {
      newErrors.formats = 'Select at least one ad format.';
      valid = false;
    }
    setErrors(newErrors);
    return valid;
  ...
  const resetErrors = () => setErrors({ offer: '', usps: '', formats: '' });
  ...
  const handleNext = async () => {
    ...
    const data = {
      mainOffer: formData.offer,
      usps: formData.usps.filter((u) => u.trim()),
      selectedFormats: formData.selectedFormats,
      enableDiversityOverrides: formData.enableDiversityOverrides,
    };
    if (briefId) data.briefId = briefId;
    const returnedBriefId = await saveSection('offers', data);
    if (returnedBriefId) {
      setBriefId(returnedBriefId);
      toast({ title: 'Offer & USPs Saved', description: 'Offer and USPs saved successfully!', variant: 'default' });
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="bg-white shadow overflow-hidden sm:rounded-lg">
          ...
          <div className="mt-6 space-y-8">
            ...
            <div>
              ...
              <textarea 
                ...
                className={`mt-2 block w-full rounded-md border ${errors.offer ? 'border-red-500' : 'border-gray-300'} shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-3`}
              ...
              <div className="mt-2 space-y-3">
                {formData.usps.map((usp, index) => (
                  <input 
                    ...
                    className={`block w-full rounded-md border ${errors.usps && usp.trim() === '' && index < 3 ? 'border-red-500' : 'border-gray-300'} shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm h-10 px-3`}
                    value={usp}
                    onChange={(e) => handleUspChange(index, e.target.value)}
                  />
                ))}
              </div>
              {errors.usps && <p className="text-red-500 text-xs mt-1">{errors.usps}</p>}
              <button 
                type="button" 
                onClick={addUsp}
        ...
        <div className="mx-4">
          ...
          <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              <div className="flex flex-col items-center text-center">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0h4m-10 0h4M7 20v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
        ...
        {/* Navigation Buttons with margins and bottom spacing */}
